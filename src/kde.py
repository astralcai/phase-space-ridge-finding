...
def dkde2(points, grid_x, grid_y, bins=128, bandwidth=0.2):
    ...
    bins_, edges = np.histogramdd(points, bins=(x_edges, y_edges), density=True)
    ...
    dx = sg.fftconvolve(bins_, k_dx, mode="same")
    dy = sg.fftconvolve(bins_, k_dy, mode="same")
    dxx = sg.fftconvolve(bins_, k_dxx, mode="same")
    dyy = sg.fftconvolve(bins_, k_dyy, mode="same")
    dxy = sg.fftconvolve(bins_, k_dxy, mode="same")
    dyx = sg.fftconvolve(bins_, k_dyx, mode="same")
...
def kden(points, bw, *grids):
    ...
    #     -np.linalg.norm(grids, axis=-1) ** 2 / (2 * bw ** 2))
